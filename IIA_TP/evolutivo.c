...
void crossover(pchrom parents, struct info d, pchrom offspring)
{
	...
	for (i = 0; i<d.popsize; i += 2)
	{
		if (rand_01() < d.pr)
		{
			point = random_l_h(0, d.moedas - 1);
			for (j = 0; j<point; j++)
			{
				offspring[i].p[j] = parents[i].p[j];
				offspring[i + 1].p[j] = parents[i + 1].p[j];
			}
			for (j = point; j<d.moedas; j++)
			{
...
void recombinacao_dois_pontos_corte(pchrom parents, struct info d, pchrom offspring)
{
	...
	for (i = 0; i<d.popsize; i += 2)
	{
		if (rand_01() < d.pr)
		{
			point1 = random_l_h(0, d.moedas - 2);
			point2 = random_l_h(point1 + 1, d.moedas - 1);
			for (j = 0; j<point1; j++)
			{
				offspring[i].p[j] = parents[i].p[j];
				offspring[i + 1].p[j] = parents[i + 1].p[j];
...
void mutacao_por_troca(pchrom offspring, struct info d)
{
	...
	for (i = 0; i<d.popsize; i++)
		...
		{
			do
				pos1 = random_l_h(0, d.moedas - 1);
			while (offspring[i].p[pos1] == 1);
			do
				pos2 = random_l_h(0, d.moedas - 1);
